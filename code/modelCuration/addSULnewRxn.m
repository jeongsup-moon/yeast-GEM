% This Function is for adding missing metabolites/reactions involving sulfur metabolism during fermenation into model.
% Input: model, VoSulnewRxnMatrix.tsv, VoSulnewRxnMetAnnotation.tsv, VoSulnewRxnProp.tsv, VoSulgenenames.tsv.
% NOTE: changeGeneAssociation.m is a function from cobra
%       Extract model info from .tsv format.
%       Before running the codes below, the files should be manually edited.
%       COBRA required.
...
%newreaction:
fid = fopen('../data/modelCuration/VoSulnewRxnMatrix.tsv');
...
% add met annotation
fid = fopen('../../data/modelCuration/VoSulnewRxnMetAnnotation.tsv');
...
for i = 1:length(newmet.metNames)
    ...
    for j = 1:length(mets)
        if mets(j)
            ...
            model.metNotes{metID}    = 'NOTES: added for sulfur metabolic pathway expansion (sulfur volatiles) (PR #296)';
        end
    end
end

%Load rxnProp(rev and GPR)
fid = fopen('../../data/modelCuration/VoSulnewRxnProp.tsv');
...
for i = 1:length(newrxn.ID)
    ...
    model.rxnNotes{rxnIndex} = 'NOTES: added for sulfur metabolic pathway expansion (sulfur volatiles) (PR #296)';
    model.subSystems{rxnIndex} = strsplit(newrxn.subSystems{i},';');
end


% add gene standard name for new genes
fid = fopen('../../data/databases/VoSulgenenames.tsv');
